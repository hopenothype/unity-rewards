...
#   - Upload boxes.csv (must include: phoneID, licenseId, boxId, boxType, SharedIPs)
...
# Apply clipboard paste if requested
...
def fetch_rewards(bearer_token, api_key, page_size=1000):
    ...
    while True:
        ...
        msg = f"Page {page}: fetched {batch_count} records (total so far: {len(all_rows) + batch_count})"
        print(f"[DEBUG] {msg}")
...
if run_btn:
    try:
        ...
        # ------------------------------------------------------------
        # TOP 10 + BOTTOM 10 REWARDS
        # ------------------------------------------------------------
        st.markdown("---")
        st.subheader("Top 10 and Bottom 10 rewards (within selected period)")

        rewards_display = df_merged[["date", "phoneID", "boxId", "boxType", "amountUnits"]].copy()
        rewards_display = rewards_display.rename(columns={"amountUnits": "rewardAmount($)"})

        col_top, col_bottom = st.columns(2)

        with col_top:
            st.markdown("**Top 10 highest rewards**")
            top10 = rewards_display.sort_values("rewardAmount($)", ascending=False).head(10)
            st.dataframe(top10, use_container_width=True)

        with col_bottom:
            st.markdown("**Bottom 10 lowest rewards** (non-zero)")
            bottom10 = (
                rewards_display[rewards_display["rewardAmount($)"] > 0]
                .sort_values("rewardAmount($)", ascending=True)
                .head(10)
            )
            if len(bottom10) == 0:
                st.info("No non-zero rewards found in the selected period.")
            else:
        ...
        with col_hist:
            fig_hist = px.histogram(
                ...
                x="amountUnits",
                nbins=50,
                title="Reward amount distribution ($)",
                labels={"amountUnits": "Reward amount ($)"}
            )
